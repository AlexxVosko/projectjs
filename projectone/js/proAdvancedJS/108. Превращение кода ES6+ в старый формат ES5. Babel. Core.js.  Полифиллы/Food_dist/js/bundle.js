...
function calc() {
  ...
  let sex, height, weight, age, ratio; //проверка из localStorage
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // общая функция подсчетов
  ...
  calcTotal(); //функция получени данных из div + выставление активности
  ...
  getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active'); //функция обработки input калькулятора

  function getDynamicInformation(selector) {
    const input = document.querySelector(selector); //проверка на конкретный input по id
...
function cards() {
  ...
  class MenuCard {
    ...
    render() {
      const element = document.createElement('div'); // проверка если в rest ничего не передано
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostDataJson(form) {
    form.addEventListener('submit', e => {
      e.preventDefault(); // Изображние загрузки
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); //postData('server.php',JSON.stringify(object))
...
/* harmony export */   "openModel": function() { return /* binding */ openModel; }
/* harmony export */ });
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  window.addEventListener('scroll', showModalByScroll); //},{once:true}); // как вариант но не всегда подходит
  // функция всплытия модального окна только один раз при прокрутке вниз

  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      // прокрученная часть + видимая часть окна  >= полная высота всей страницы
...
function slider(_ref) {
  ...
  getNum(slides.length, total, current); //функция активности точек
  ...
  slidesWrapper.style.overflow = 'hidden'; // установка ширины для каждого слайда
  ...
  dots.style.cssText = `
        ...
        list-style: none;
    `;
  slider.append(dots); // создание точек от количества слайдов
  ...
  next.addEventListener('click', () => {
    ...
    if (offset == replaceWidth(width) * (slides.length - 1)) {
      // условия для последнего слайда
      offset = 0;
    } else {
      // если не последний слайд - добавить смещение
    ...
    getNum(slides.length, '', current); //установка активности точек
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  const tabs = document.querySelectorAll(tabsSelector),
        ...
        tabsParent = document.querySelector(tabsParentSelector); // функция скрытия табов
  ...
  hideTabContent();
  showTabContent(); // обработчик меню
...
function timer(id, deadline) {
  ...
  function setClock(selector, endtime) {
    const timer = document.querySelector(selector),
          ...
          timeInterval = setInterval(updateClock, 1000); // решение мигание верстки
...
const getResource = async url => {
  const res = await fetch(url); // обработка ошибок http
...
window.addEventListener("DOMContentLoaded", () => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModel)('.modal', modalTimerId), 50000);
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2022-06-12');
